...
- 이미지 ID: `sha256:4c304e397d72c336257fcc5831872362cd52a6953f9fa3774ec99e0337a013be`.
...
| 검증 | 결과 |
|---|---|
| `make check` | fmt, Clippy `-D warnings`, Rust 테스트 87개 모두 통과 |
...
HTTP 테스트는 실제 임시 loopback 포트를 사용한다. 제한된 샌드박스 안에서는 포트 바인딩이 거절되어 해당 5개가 실패했고, 로컬 포트를 허용한 환경에서 전체 87개가 통과했다. 이 환경 제약을 테스트 제외로 처리하지 않았다.
