...
6. 기획·재사용·시간 모델 변경은 `make lifecycle-check`로 30단계 이력과 재시작/복원 후 재조회를 검증한다. 상세 범위는 `docs/lifecycle.md`를 따른다.
7. 발견 결함을 수정하고 영향을 받는 검증을 재실행한다. 남은 한계와 실제 측정 조건을 기록한다.
...
인증 정보는 `.env`와 클라이언트 환경에 보관하며 커밋하지 않는다. 원격 저장소는 후속 사용자 요청으로 연결되었으며, Git에 실행 중 DB 데이터나 원본 개인 로그를 포함하지 않는다.

## 30단계 검증 확장

Main은 시간 조회 수정, Docker 하네스, 문서와 최종 실행을 담당한다. Sol은 `tests/lifecycle.py`와 세 버전의 입력 기획서, Terra는 외부 의존성 없는 `tests/lifecycle_workloads.py`를 담당한다. Main이 실행 코드와 결과를 검토하며, 작은 기술 실험 통과를 제품 기능 전체의 완성으로 해석하지 않는다.

## 원자화·문맥 근거 확장

Grok/Fable은 독립 설계 검토와 실제 원자화 출력을 제공했다. Terra는
`tests/integrity_audit.py` 및 검사 자체의 반례 테스트를 작성했고 Main이 데이터 계약과
실제 Neo4j에 대조했다. Main은 문맥 비교, 모델 출력 파싱/검토, MCP roundtrip,
의미 반례, Docker 통합과 문서를 담당했다. `make evidence-check`로 기존 회귀와
확장 시나리오를 함께 실행한다. 생성 모델 호출은 자동화하지 않고, 검토 이력이
명시된 합성 응답 fixture를 재생한다. 출력 경로를 재사용하거나 실패 로그를 삭제하지 않는다.
