...
> 문서·스크린샷 기준: 2026-09-07, 메인 `eb542a4`. Project·Schema·Core의 목표 저장 모델은 구현되어 있습니다. 이 기준 버전의 화면은 Schema 목표·수치 기준·평가 상태를 보여줍니다. 각 계층의 AI 예상 달성률(%)과 전체 버전 계보에서 두 버전을 고르는 UI는 별도 보완 중이며, 아래에서 완료 기능과 설계 의도를 구분합니다.
...
현재 저장소는 로컬 Git 저장소이며 공개 원격 주소를 전제로 하지 않습니다. 소스를 받은 뒤 저장소 루트에서 명령을 실행합니다. Windows에서는 아래 POSIX 셸 명령을 실행할 수 있는 환경이 필요하며, 현재 실검증 환경은 macOS Docker Desktop입니다.
...
**달성률 표시의 상태:** 이 문서의 화면 기준에서는 수치 목표와 충족/미충족/미측정 상태를 볼 수 있습니다. “Project·Schema·Core 각각의 예상 목표를 현재 아이디어·구현·검증과 비교한 AI 예상 달성률(%)”은 추가 보완 대상입니다. 목표 임계값, 실측 성능, AI가 추정한 진행 정도, 성공 확률은 서로 다른 정보로 다뤄야 합니다. 해당 기능의 확정된 계약과 수용 결과는 [API 계약](docs/api.md)과 [검증 기록](docs/validation.md)을 기준으로 확인하세요.
...
본문 삭제·추가를 강조하고, 연결된 하위 항목과 역할의 변경을 보여줍니다. 비교 대상은 실제 이전 Revision 또는 명시적 파생 관계에서 찾습니다. 이전 후보가 여러 개면 선택합니다. 현재 화면 기준의 비교는 이전 후보와 선택 버전 사이이며, **전체 v1 → v2 → v3 체인에서 임의의 두 버전을 선택하는 기능은 보완 중**입니다.
...
3D 그래프 전체를 볼 수 있는 조작 검증 화면입니다. 이 이미지는 최신 요약 카드 추가 전의 `213faab` UI이며, 탐색 축과 조작은 동일합니다. 위 두 이미지는 `eb542a4` 기준입니다.
...
검증 기준 `eb542a4`에서 Rust 124개, 실DB 수용 13개, MCP 입력 12개, 메인 UI 8+11개를 통과했습니다. 재시작·빈 볼륨 복원·프로세스 실패 전파·정상 종료도 별도로 확인했습니다. 실행별 성공·실패 로그와 한계는 [검증 기록](docs/validation.md)에 남깁니다. 문서 작성 자체가 새 설치나 테스트 전체의 재실행을 뜻하지는 않습니다.
