...
- 모든 수준과 Idea의 사용 위치에 목표를 둘 수 있다. Schema 자체 목표와 하위 Idea 목표를 구분한다. 누락된 목표는 허용하고 대시보드에서 누락으로 표시한다. 사용자가 목표 기획을 요청하면 로컬 AI 클라이언트가 현재 root/사용 경로에 고정한 목표와 정량 기준을 제안하고 MCP로 저장한다. DB나 대시보드가 LLM을 호출하지 않는다.
...
- 버전 비교는 실제 이전 Revision 또는 명시적 파생 관계를 사용한다. 본문 추가/삭제, 구성 연결·역할·메타데이터 변경과 기록된 변경 이유를 표시한다. 이전 후보가 여럿이면 사용자가 비교 대상을 선택한다.
- 목표는 현재 범위의 공식 실측과 AI 제안을 분리하고, 수치 기준·관측값·미측정 상태를 표시한다. 과거 버전의 실적을 새 목표에 자동 승계하지 않으며, 평가가 없는 새 기준선은 미측정이다.
...
| 2D·3D 화면 | Terra/Sol/Main | 읽기 전용 탐색·검색·전체 이력·원문·시간/깊이/스키마 그래프 | 메인 요약·diff·목표 8개 + 기존 탐색·3D 11개 통과 |
| Docker/통합 검증 | Main | 실제 단일 이미지 및 재시작·복원 테스트 | 실제 이미지·재시작·복원·종료 검증 완료 |
| 독립 수용 검토 | Sol/Main | 주요 불변 조건 검토, 발견 결함 해결 | Rust 124개·실DB 13개·MCP intake 12개 및 실제 조회 취소 회귀 통과 |
