...
상태: MVP 구현 및 30단계 수용 검증 완료 · 2026-09-07 · 설계 책임/통합 검증: Main(Codex)
...
원문 저장 → 수동 또는 로컬 AI가 구조화 → 후보/패키지 검토 → 결정적 검증 → 배치 적용.
...
- 선택 Project/스냅샷/시간에 속하는 불변 본문을 대상으로 텍스트와 벡터 검색 결과를 순위 결합한다. 임베딩은 외부에서 공급하며 모델/차원을 고정한다. 임베딩이 없으면 텍스트 검색으로 동작하고 상태를 표시한다.
...
- MVP 2D: 재귀 구성 탐색, 원문/사용 위치/계보/근거 상세, 작업/공식 버전과 시점 조회, 역할 검색, 기준별 평가 대시보드, 원문 및 패키지 검토/적용.
- 후속 3D: X=기록/발생 시간, Y=선택 경로의 깊이, Z=Schema 또는 관점 레이어. 좌표는 저장 모델의 정체성이 아니라 투영이다. 동일 노드는 여러 위치에 표시될 수 있다. 전체 그래프를 무제한 렌더링하지 않는다.
...
| 2D 화면 | Terra | 고정 API 계약에 연결된 탐색/입력/검토 | 핵심 입력/조회 브라우저 검증 완료 |
| Docker/통합 검증 | Main | 실제 단일 이미지 및 재시작·복원 테스트 | 실제 이미지·재시작·복원·종료 검증 완료 |
| 독립 수용 검토 | Sol/Main | 주요 불변 조건 검토, 발견 결함 해결 | 88개 테스트·실DB 12개 시나리오 통과 |
| 30단계 수명주기 | Main/Sol/Terra | 기획 전환·역할 실행·시간/검색/근거·복원 후 재조회 | 347개 기록·30개 체크포인트·재시작/복원 검증 통과 |

## 후속 범위

3D, 실제 팀 권한/HA, 외부 아티팩트 보관 서비스, 대규모 벡터 인덱스 최적화, 자율 에이전트 실행, 전역 의미 병합, 자동 인과 영향 전파, 사용자 저장 엔진. Neo4j를 대체할지는 실제 병목을 측정한 뒤 결정한다.
