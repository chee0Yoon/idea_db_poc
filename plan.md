...
상태: MCP 전용 입력·2D/3D 대시보드·버전 비교·스키마 목표 요약 및 메인 UI 검증 완료 · 2026-09-07 · 설계 책임/통합 검증: Main(Codex)

실험 확장: Grok/Fable 실제 원자화 출력과 MCP 저장, 39개 질의의 문맥 포장 비교, 독립 Neo4j 무결성 검증을 추가했다. [근거·반례·제한](docs/context-evidence.md)을 따른다. 의미적 원자성·목표 적합성·LLM 답변 품질을 저장 성공이나 예상 달성률로 대체하지 않는다.
...
- 모든 수준과 Idea의 사용 위치에 목표를 둘 수 있다. Project·Schema·재귀 Core 각각의 자체 목표와 하위 Idea 목표를 구분한다. 누락된 목표는 허용하고 대시보드에서 누락으로 표시한다. 사용자가 목표 기획을 요청하면 로컬 AI 클라이언트가 현재 root/사용 경로에 고정한 목표와 구체적인 기준을 제안하고 MCP로 저장한다. 현재 원자 계획·설계·구현·검증 기록을 대조한 기준별 AI 예상 달성률도 근거·산정 방법·cutoff와 함께 별도 기록한다. DB나 대시보드가 LLM을 호출하지 않는다.
...
- 첫 화면은 현재 프로젝트와 Schema별 목표·필수 기준 상태를 요약한다. ID·해시·원시 JSON은 접힌 기술 정보에서 확인한다. 트리/요약의 Schema를 선택하면 해당 상세와 목표가 즉시 보이며, 최신 구성 복귀는 과거 시점 필터를 해제한다.
- 버전 비교는 실제 이전 Revision 또는 명시적 파생 관계의 양방향 계보에서 시작·끝 버전을 선택한다. 오래된 항목에서도 현재 지식 시점 안의 후속 버전으로 이동할 수 있다. 각 버전의 변경 이유·원문·관측·목표·평가를 항목별 이력으로 표시하며 발생 시점 필터를 적용한다. 본문 추가/삭제, 구성 연결·역할·메타데이터 변경과 기록된 변경 이유를 표시한다. 이전 후보가 여럿이면 사용자가 비교 대상을 선택한다.
- 목표는 현재 범위의 공식 실측과 AI 제안을 분리하고, 예상 목표·기준별 근접도·AI 예상 달성률·근거와 남은 작업을 표시한다. 이정표 추정은 성능 측정이나 성공 확률이 아니다. 같은 목표의 필수 기준 모두에 평가가 있을 때만 같은 비중으로 평균하며, 자식의 수나 점수를 부모 달성률로 자동 평균하지 않는다. 과거 버전의 실적을 새 목표에 자동 승계하지 않으며, 평가가 없는 새 기준선은 미측정이다.
...
- 실제 API에 연결된 2D/3D 화면, Project·Schema·Core 자체 목표의 AI 예상 달성률과 버전 범위 비교. 정적 파일을 Rust 서비스에서 제공한다.
- 입력 스킬, API 계약, 재현 가능한 예제와 검증 스크립트, CI, AGENTS.md, 작업 소유권/수용 기준.
- 사용자 기존 Python 실험판은 변경하지 않는다. 개발 작업은 임시 격리 경로에서 수행하고 최종 저장소는 `idea_db_neo4j`에 둔다. 저장소/제품 이름은 `idea_db`이며, 후속 사용자 요청으로 원격 `chee0Yoon/idea_db_poc`에 연결했다.
...
8. `make evidence-check`: 기존 30단계에 실제 모델 검토 출력 24개 사례, 원문/원자/원문 주변/부모/의존 문맥 비교 39개 질의, Unicode·의미 반례 10개와 전체 export/Neo4j 관계 무결성 검사를 추가한다. 필요한 근거 보존율은 생성 답변 정확도와 구분하고, 작은 예산에서 문맥 확장이 악화되는 결과도 남긴다.
...
| 2D·3D 화면 | Terra/Sol/Main | 읽기 전용 탐색·검색·전체 이력·원문·시간/깊이/스키마 그래프 | 메인 계층별 목표·예상 달성률·버전 계보 및 기존 요약·3D 검증 |
| Docker/통합 검증 | Main | 실제 단일 이미지 및 재시작·복원 테스트 | 실제 이미지·재시작·복원·종료 검증 완료 |
| 독립 수용 검토 | Sol/Main | 주요 불변 조건 검토, 발견 결함 해결 | Rust 129개·실DB 13개·MCP intake 12개 및 실제 조회 취소 회귀 통과 |
